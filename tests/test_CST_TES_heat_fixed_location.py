...
class TestMasterHeat(unittest.TestCase):
    ...
    def test_Pilbara(self):
        ...
        region='Pilbara'
        #best=get_best_location(2020) # when chosing location by best H2 price
        #location=region+' %s'%best[region] # ""
        location = region + ' 3' # fixed to location Pilbara 3 (best H2 price)
        print(location)
        solcast_TMY=False
          
        LCOH, CF, C_cap=master(self.model_name, location, RM=self.RM, t_storage=self.t_storage, P_load_des=self.P_load_des, casedir=self.casedir, OM_method='SL', verbose=True, solcast_TMY=solcast_TMY)
        ...
        self.assertTrue(balance_1<1e-2)
        self.assertTrue(balance_2<1e-2)
        self.assertTrue(abs(LCOH-32.156)/32.156<0.05)
        self.assertTrue(abs(CF-0.6)/0.6<0.05)
        self.assertTrue(abs(C_cap- 871089240.6)/ 871089240.6<0.05)

        #os.system('rm -r %s'%self.casedir)    

    def test_Pilbara_modules(self):
        ...
        LCOH, CF, C_cap=master(self.model_name, location, RM=self.RM, t_storage=self.t_storage, P_load_des=self.P_load_des, casedir=self.casedir, OM_method='SL', verbose=True, solcast_TMY=solcast_TMY, multi_cst_modules=True)
